...
EstimateCurvature <- function(D,
  ...
  mid.search <- optimal_midpoint_search(D, top.k = J, d.yz.min = min(d.yz.min,
                                                                     max(D)/2), d.yz.max = d.yz.max)
  if (verbose) {
    print("Midpoints: ")
  ...
  if (!any(is.na(opt.vec))) {
    x.set <- filter_indices(D, y.opt, z.opt, m.opt, tri.const = tri.const)
    kappa.set <- EstimateKappaSet(D, y.opt, z.opt, m.opt,
                                  x.set)
    out.set <- list(kappas = kappa.set, kappa.med = median(kappa.set,
                                                           na.rm = T), midpoints = mid.search)
  }
  else {
    out.set <- list(kappas = NULL, kappa.med = NULL, midpoints = mid.search)
  }
  return(out.set)
}
